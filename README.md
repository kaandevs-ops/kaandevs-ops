<div align="center">

<div align="center">
  <img src="banner.png" alt="Kaan Tekin — AI, Robotics & Autonomous Systems" width="100%" />
</div>

<img src="https://readme-typing-svg.demolab.com/?font=Fira+Code&size=20&pause=1000&color=B388FF&center=true&vCenter=true&width=650&lines=Information+Systems+%26+Technologies+Student;AI+%2B+Robotics+%2B+Computer+Vision;Currently+building%3A+smart-factory-orchestrator;Freelance+%7C+Open-source+%7C+Turkey+%F0%9F%87%B9%F0%9F%87%B7" alt="Typing SVG" />

<br/>

![Student](https://img.shields.io/badge/Information%20Systems%20%26%20Technologies%20Student-2C1E4A?style=for-the-badge&logoColor=white)
![AI/Robotics](https://img.shields.io/badge/AI%20%26%20Robotics-3A2A5D?style=for-the-badge&logoColor=white)
![Location](https://img.shields.io/badge/Türkiye-4B0082?style=for-the-badge&logo=googlemaps&logoColor=white)

[![Email](https://img.shields.io/badge/Email-4B0082?style=for-the-badge&logo=icloud&logoColor=white)](mailto:kaantekin06@icloud.com)
[![GitHub](https://img.shields.io/badge/GitHub-2E1A47-black?style=for-the-badge&logo=github&logoColor=white)](https://github.com/kaandevs-ops)

![Profile Views](https://komarev.com/ghpvc/?username=kaandevs-ops&color=6a0dad&style=for-the-badge&label=PROFILE+VIEWS)
![Followers](https://img.shields.io/github/followers/kaandevs-ops?style=for-the-badge&color=8A2BE2&labelColor=1a1a2e)
![Stars](https://img.shields.io/github/stars/kaandevs-ops?style=for-the-badge&color=B388FF&labelColor=1a1a2e)

</div>

<br/>

## 🟣 About Me

I'm **Kaan**, an Information Systems & Technologies student in Türkiye focused on building practical software systems around **AI, backend development, robotics, and real-time data**.

I enjoy working on projects where multiple systems have to work together — from AI-powered applications and event-driven backends to real-time data pipelines and simulation.

- 🔭 Currently building **smart-factory-orchestrator** — an event-driven platform for industrial orchestration, predictive maintenance, AGV coordination, and digital twin simulation
- 🤖 Building and experimenting with **AI-powered software, local LLMs, computer vision, and automation**
- 🛰️ Exploring **robotics, embedded systems, and sensor fusion**
- 🛠️ Interested in building **end-to-end software systems** that solve practical problems
- 📫 kaantekin06@icloud.com

**Open to:** freelance software projects, AI/backend work, internships, junior opportunities, and open-source collaboration.

---

<img src="https://raw.githubusercontent.com/kaandevs-ops/kaandevs-ops/output/github-contribution-grid-snake-dark.svg" width="90%" />

## 🟣 Tech Stack

<div align="center">

**Languages**
<br/>
<img src="https://skillicons.dev/icons?i=python,cpp,cs,java,js,kotlin&theme=dark" />

**Frontend**
<br/>
<img src="https://skillicons.dev/icons?i=html,css,react&theme=dark" />

**Backend & Databases**
<br/>
<img src="https://skillicons.dev/icons?i=nodejs,dotnet,mysql,sqlite&theme=dark" />

**Cloud, DevOps & Tooling**
<br/>
<img src="https://skillicons.dev/icons?i=git,github,arduino,unity,unreal&theme=dark" />

</div>

---

## 🟣 What I Work With

```text
Languages
  Python · C# · TypeScript · Java · C++ · Kotlin

AI / ML
  LLM applications · RAG · Computer vision · Local / offline inference

Backend & Systems
  REST APIs · Event-driven architecture · Real-time data pipelines · Automation

Currently exploring
  Robotics · Sensor fusion · Distributed systems · Digital twins
```

---

## 🟣 Featured Projects

<details open>
<summary><b>🏭 smart-factory-orchestrator</b></summary>
<br/>

An actively developed event-driven orchestration platform exploring predictive maintenance, AGV coordination, and digital twin simulation through a shared software architecture.

| | |
|---|---|
| **Stack** | Python, event-driven architecture, multi-LLM core |
| **Focus** | Keeping orchestration, robotics coordination, and analytics decoupled but talking to each other |
| **Repo** | [smart-factory-orchestrator](https://github.com/kaandevs-ops/smart-factory-orchestrator) |

</details>

<details>
<summary><b>🛰️ SENTINEL</b></summary>
<br/>

A real-time geospatial visualization project — pulls in aircraft, satellite, weather, and other public data feeds and renders them on a 3D globe.

| | |
|---|---|
| **Stack** | Python, 3D visualization, live data aggregation |
| **Focus** | Pulling from 12+ public sources and keeping the render smooth as data streams in |
| **Repo** | [SENTINEL](https://github.com/kaandevs-ops/SENTINEL) |

</details>

<details>
<summary><b>🤖 kodai</b></summary>
<br/>

A Turkish-language coding assistant that runs fully offline on Apple MPS using Qwen2.5-Coder — no API calls, no cloud dependency.

| | |
|---|---|
| **Stack** | Python, Qwen2.5-Coder, Apple MPS |
| **Focus** | Proving a genuinely useful coding assistant doesn't need a cloud backend |
| **Repo** | [kodai](https://github.com/kaandevs-ops/kodai) |

</details>

<details>
<summary>More projects</summary>
<br/>

**📡 turkiye-radar** — Interactive map tracking earthquakes, weather, and other public events across Turkey, with AI-assisted summaries. [Repo](https://github.com/kaandevs-ops/turkiye-radar)

**💬 html-ai** — A local, memory-persistent AI assistant with Telegram/WhatsApp integration, running entirely on Ollama. [Repo](https://github.com/kaandevs-ops/html-ai)

</details>

---

## 🟣 Connect

<div align="center">

[![Gmail](https://img.shields.io/badge/Email-kaantekin06%40icloud.com-4B0082?style=for-the-badge&logo=icloud&logoColor=white)](mailto:kaantekin06@icloud.com)
[![GitHub](https://img.shields.io/badge/GitHub-kaandevs--ops-2E1A47?style=for-the-badge&logo=github&logoColor=white)](https://github.com/kaandevs-ops)

</div>

---

<div align="center">

*Still learning, still shipping.*

![Footer](https://capsule-render.vercel.app/api?type=waving&color=0:24243E,50:302B63,100:0F0C29&height=120&section=footer)

</div>
